...
public class SaveLoad {
    ...
    public static void load(GameState s) {
        ...
        try (BufferedReader r = Files.newBufferedReader(SAVE)) {
            ...
            // Исправлено: безопасный разбор данных игрока
            String playerData = map.getOrDefault("player", "Герой;20;5");
            String[] pp = playerData.split(";");
            
            // Проверяем количество частей
            if (pp.length >= 3) {
                p.setName(pp[0]); // Имя теперь первое
                p.setHp(Integer.parseInt(pp[1]));
                p.setAttack(Integer.parseInt(pp[2]));
            } else {
                // Значения по умолчанию если данных недостаточно
                p.setName("Герой");
                p.setHp(20);
                p.setAttack(5);
                System.out.println("Восстановлены значения по умолчанию для игрока");
            }
            ...
            // Восстанавливаем комнату (упрощенно)
            String roomName = map.getOrDefault("room", "Площадь");
            // Здесь нужно добавить логику восстановления текущей комнаты
            // Пока просто выводим информацию
            System.out.println("Текущая комната: " + roomName);
            
            System.out.println("Игра загружена. Игрок: " + p.getName() + ", HP: " + p.getHp() + ", Атака: " + p.getAttack());
        } catch (IOException e) {
            throw new UncheckedIOException("Не удалось загрузить игру", e);
        } catch (NumberFormatException e) {
            System.out.println("Ошибка формата данных в сохранении: " + e.getMessage());
        } catch (Exception e) {
            System.out.println("Непредвиденная ошибка при загрузке: " + e.getMessage());
        }
    }
